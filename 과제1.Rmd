...
fit_krr <- function(X, y, lambda = 0.0001, rho = 1){
  ...
  # 정규화 항 계산: K + lambda * I_n
  K_reg <- K + lambda * diag(n)
  
  # 계수 alpha 계산: alpha = (K + lambda * I_n)^(-1) * y
  alpha <- solve(K_reg, y)
  
  structure(list(alpha = alpha, X_train = X, y_train = y, lambda = lambda, rho = rho), class = 'krr')
}
```
...
predict 함수 확장 구현:
```{r}
predict.krr <- function(obj, X_new, ...){
  
  # 파라미터 추출
  alpha <- obj$alpha
  X_train <- as.numeric(obj$X_train)
  rho <- obj$rho
  
  sapply(X_new, function(x){
    k_x_X <- exp(- rho * (x - X_train)^2)
    sum(k_x_X * alpha)
  })
}
```
...
plot.krr <- function(x, ...) {
    ...
    # X_train의 최소/최대값 기준으로 100개의 새로운 X 값 생성
    X_new <- seq(min(X_train), max(X_train), length.out = 100)
    
    # 예측값 계산
    Y_pred <- predict.krr(model, X_new) 
